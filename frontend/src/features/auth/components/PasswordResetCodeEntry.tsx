...
export const PasswordResetCodeEntry = () => {
	...
	const onSubmit = async (values: ResetCodeFormData) => {
		if (!email) {
			toast.error(
				"Email address not available. Please start the password reset process again."
			);
			navigate("/auth/login");
			return;
		}
		...
		logger.info("Reset code verification submitted", {
			code: values.code,
			email,
		});
		...
		try {
			...
			// Use hard navigation (not React Router) to avoid the route guard
			// race condition — tokens were just stored and the guard's async
			// getCurrentUser() may not see them yet on a client-side transition.
			...
			if (
				...
				form.setError("code", {
					type: "manual",
					message:
						"This code has already been used. Please request a new password reset.",
				});
			} else {
				...
				<div className="text-center">
					...
					{isDuplicate && (
						<div className="mt-2 p-2 bg-blue-50 border border-blue-200 rounded text-xs text-blue-800">
							<p>
								<strong>Note:</strong> No new email was sent. Please check your
								existing email for the 4-digit code.
							</p>
						</div>
					)}
				</div>
