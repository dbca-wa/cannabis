...
>(
	(
		...
		const handleOfficerCreated = (newOfficer: PoliceOfficerTiny) => {
			// Auto-select the newly created officer
			onValueChange(newOfficer.id);
		};

		const handleExternalCreateOfficer = () => {
			setShowExternalCreateModal(true);
		};

		const handleExternalOfficerCreated = (newOfficer: PoliceOfficerTiny) => {
			// Auto-select the newly created officer and close dropdown if open
			onValueChange(newOfficer.id);
			setOpen(false);
		};
		...
		return (
			<>
				...
				<CreateOfficerModal
					...
					onCreate={handleOfficerCreated}
				/>

				{/* External Create Officer Modal */}
				<CreateOfficerModal
					open={showExternalCreateModal}
					onOpenChange={setShowExternalCreateModal}
					onCreate={handleExternalOfficerCreated}
				/>
			</>
		);
	}
