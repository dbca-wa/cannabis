...
class SystemSettingsService {
	...
	private optimisticUpdates: Map<string, string | boolean> = new Map();
	...
	private pendingRequests: Map<string, Promise<ServiceResult<SystemSettings>>> =
		new Map();
	...
	async getSettings(
		...
		try {
			...
			if (Date.now() < this.rateLimitedUntil) {
				...
				logger.warn(
					`Settings service is rate limited, returning cached data if available`
				);
			...
			const requestPromise = this.makeApiCall<SystemSettings>(() => {
				logger.debug("Making API call", {
					endpoint: ENDPOINTS.SYSTEM.SETTINGS,
				});
	...
	async updateSetting(
		...
		return this.updateSettings(
			{ [field]: value } as SystemSettingsUpdateRequest,
			options
		);
	}
	...
	validateSettings(settings: Partial<SystemSettings>): {
		...
		if ("tax_percentage" in settings) {
			const validation = this.validateTaxPercentage(
				settings.tax_percentage as string
			);
			if (!validation.isValid) {
				errors.push(validation.error);
		...
		if ("forward_certificate_emails_to" in settings) {
			const validation = this.validateEmail(
				settings.forward_certificate_emails_to as string
			);
			if (!validation.isValid) {
				errors.push(validation.error);
	...
	private async makeApiCall<T>(
		...
		if (Date.now() < this.rateLimitedUntil) {
			const waitTime = Math.ceil((this.rateLimitedUntil - Date.now()) / 1000);
			logger.warn(
				`Circuit breaker active for ${operation}, rate limited for ${waitTime} more seconds`
			);
			return {
				data: {} as T,
				success: false,
		...
		for (let attempt = 1; attempt <= this.config.retryAttempts; attempt++) {
			try {
				...
				return {
					data: response as T, // apiClient.get returns data directly, not wrapped
					success: true,
				};
			} catch (error: unknown) {
				lastError = error;
				...
				if (status === 429) {
					...
					if (retryAfter) {
						...
						waitTime = Math.min(
							60 * Math.pow(2, this.consecutiveRateLimits - 1),
							300
						); // Max 5 minutes
					}
					...
					logger.warn(
						`Rate limit hit for ${operation}, circuit breaker active for ${waitTime} seconds`,
						{
				...
				if (status !== undefined && status >= 400 && status < 500) {
					...
					if (status !== 408) {
						// 408 Request Timeout can be retried
						break;
					}
				}
		...
		// Handle final error
		const errorWithResponse = lastError as {
			response?: { status?: number; data?: SettingsValidationError };
		};
		if (
			errorWithResponse?.response?.status === 400 &&
			errorWithResponse?.response?.data?.errors
		) {
			// Validation error from backend
			const validationError = errorWithResponse.response.data;
			return {
				data: {} as T,
				success: false,
		...
		return {
			data: {} as T,
			success: false,
			error: getErrorMessage(lastError) || "Unknown error occurred",
		};
	}
	...
	private validatePricingField(
		...
		if (!value || value.trim() === "") {
			return {
				isValid: false,
				error: `${this.getFieldDisplayName(field)} is required`,
			};
		}
		...
		if (isNaN(numValue)) {
			return {
				isValid: false,
				error: `${this.getFieldDisplayName(field)} must be a valid number`,
			};
		}
		...
		if (numValue < 0) {
			return {
				isValid: false,
				error: `${this.getFieldDisplayName(field)} cannot be negative`,
			};
		}
		...
		if (numValue > 999999.99) {
			return {
				isValid: false,
				error: `${this.getFieldDisplayName(field)} must not exceed $999,999.99`,
			};
		}
		...
		if (decimalPlaces > 2) {
			return {
				isValid: false,
				error: `${this.getFieldDisplayName(field)} can have at most 2 decimal places`,
			};
		}
	...
	private validateTaxPercentage(value: string): {
		...
		if (decimalPlaces > 2) {
			return {
				isValid: false,
				error: "Tax percentage can have at most 2 decimal places",
			};
		}
