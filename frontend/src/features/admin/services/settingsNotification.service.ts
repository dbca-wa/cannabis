...
class SettingsNotificationService {
	...
	handleSettingsChange(
		...
		try {
			...
			const historyEntry: ChangeHistoryEntry = {
				...
				environment,
				isCritical,
				formattedOldValue: this.formatValue(
					notification.field,
					notification.oldValue
				),
				formattedNewValue: this.formatValue(
					notification.field,
			...
			logger.error("Error handling settings change notification", {
				error,
				notification,
			});
		}
	...
	showUpdateSuccess(
		...
		toast.success(message, {
			duration: 4000,
			description: user
				? `Updated by ${this.getUserDisplayName(user)}`
				: undefined,
		});
	...
	showUpdateError(
		...
		if (failedFields && failedFields.length > 0) {
			const fieldNames = failedFields.map((field) =>
	...
	showEnvironmentWarning(
		...
		if (envLower === "production") {
			toast.warning(`Production ${action}`, {
				duration: 8000,
				description:
					details || "Changes will affect live operations immediately",
			});
	...
	updateConfig(config: Partial<NotificationConfig>): void {
		...
		logger.debug("Settings notification config updated", {
			config: this.config,
		});
	}
