...
export const EmailSettingsCard: React.FC<EmailSettingsCardProps> = ({
    ...
    const getEmailRoutingStatus = () => {
        ...
        if (sendToSelf) {
            return {
                ...
                badge: <Badge variant="outline" className="text-orange-600 border-orange-600">Testing</Badge>
            };
        } else {
            return {
                ...
                badge: <Badge variant="outline" className="text-green-600 border-green-600">Live</Badge>
            };
        }
    };
    ...
    const sendTestEmail = async () => {
        try {
            ...
            showSuccess(`Test email sent to ${settings.forward_certificate_emails_to}`);
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="p-4 border rounded-lg bg-muted/50 transition-all duration-200 hover:bg-muted/70">
                                ...
                                <div className="flex items-center justify-between">
                                    <div className="space-y-1">
                                        <Label htmlFor="email-toggle">Send emails to admin (testing mode)</Label>
                                        <p className="text-xs text-muted-foreground">
                                            When enabled, all system emails will be sent to the admin email instead of actual recipients
                                        </p>
                                    </div>
                                    <Switch
                                        id="email-toggle"
                                        ...
                                        disabled={!settings.send_emails_to_self_editable || isSubmitting}
                                    />
                                </div>
                            </div>
                            ...
                            {!settings.send_emails_to_self_editable && (
                                <Alert>
                                    ...
                                    <AlertDescription>
                                        Email routing cannot be changed in the {settings.environment} environment.
                                        {settings.environment.toLowerCase() === 'local' || settings.environment.toLowerCase() === 'development'
                            ...
                            <div className="space-y-3">
                                    ...
                                    <TooltipProvider>
                                        <Tooltip>
                                            ...
                                            <TooltipContent className="max-w-xs">
                                                <p>This email address receives all system notifications when testing mode is enabled</p>
                                            </TooltipContent>
                                        </Tooltip>
                                    </TooltipProvider>
                                </div>
                                <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
                                    <div className="flex-1 p-3 bg-muted rounded-md transition-colors hover:bg-muted/80 w-full">
                                        <span className="text-sm font-mono break-all">{settings.forward_certificate_emails_to}</span>
                                    </div>
                                    <Button
                                        variant="outline"
                                        size="sm"
                        ...
                        <FormField
                            ...
                            render={({ field }) => (
                                <FormItem>
                                    ...
                                    <FormDescription>
                                        Email address that receives system notifications in testing mode
                                    </FormDescription>
                                    <FormMessage />
                                </FormItem>
                            )}
