 ...
 */
export function validatePricingField(
	field: string,
	value: string
): ValidationResult {
	...
	if (config.decimalPlaces !== undefined) {
		...
		if (decimalPlaces > config.decimalPlaces) {
			const decimalText =
				config.decimalPlaces === 1 ? "decimal place" : "decimal places";
			return {
				isValid: false,
...
export function validateBoolean(
	...
	if (typeof value === "string") {
		...
		if (
			["true", "false", "1", "0", "yes", "no", "on", "off"].includes(lowerValue)
		) {
			return { isValid: true, error: "" };
		}
	}
		...
		if (field in settings) {
			...
			if (!result.isValid) {
				...
				if (result.suggestion) {
					suggestions.push(
						`${getFieldDisplayName(field)}: ${result.suggestion}`
					);
				}
			}
		}
...
export function getFieldHelpText(field: string): string {
	const helpTexts: Record<string, string> = {
		...
		forward_certificate_emails_to:
			"Email address that receives system notifications when testing mode is enabled",
		send_emails_to_self:
