		...
		<Card className="relative">
			<CardHeader className="pb-3">
				<div className="flex items-center justify-between">
					<div className="flex items-center gap-2">
						...
						<CardTitle className="text-base">Bag #{index + 1}</CardTitle>
					</div>
					<Button
						type="button"
						variant="ghost"
			...
			<CardContent className="space-y-4">
				...
				<div className="space-y-2">
					<Label htmlFor={`content_type_${index}`} className="required">
						Content Type
					</Label>
					<Select
					...
					>
						<SelectTrigger
							...
							className={getFieldError("content_type") ? "border-red-500" : ""}
						>
				...
				<div className="space-y-2">
					<Label htmlFor={`seal_tag_numbers_${index}`} className="required">
						Original Seal Tag Numbers
					</Label>
					<Input
						id={`seal_tag_numbers_${index}`}
						value={bag.seal_tag_numbers}
						onChange={(e) => onUpdate({ seal_tag_numbers: e.target.value })}
						placeholder="Enter original tag numbers"
						className={
							getFieldError("seal_tag_numbers") ? "border-red-500" : ""
						}
					/>
				...
				<div className="space-y-2">
					<Label htmlFor={`new_seal_tag_numbers_${index}`} className="required">
						New Seal Tag Numbers
					</Label>
					<Input
						id={`new_seal_tag_numbers_${index}`}
						value={bag.new_seal_tag_numbers}
						onChange={(e) => onUpdate({ new_seal_tag_numbers: e.target.value })}
						placeholder="Enter new tag numbers (if resealed)"
						className={
							getFieldError("new_seal_tag_numbers") ? "border-red-500" : ""
						}
					/>
				...
				{/* Determination */}
				<div className="space-y-2">
					<Label htmlFor={`determination_${index}`} className="required">
						Determination
					</Label>
					<Select
				...
				<div className="pt-4 border-t space-y-4">
					...
					<div className="space-y-2">
						...
						<Input
							...
							onChange={(e) => onUpdate({ property_reference: e.target.value })}
							placeholder="Enter property reference number"
							className={
								getFieldError("property_reference") ? "border-red-500" : ""
							}
						/>
					...
					<div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
						{/* Gross Weight */}
						<div className="space-y-2">
							<Label htmlFor={`gross_weight_${index}`}>Gross Weight (g)</Label>
							<Input
								id={`gross_weight_${index}`}
								type="number"
								step="0.01"
								value={bag.gross_weight}
								onChange={(e) => onUpdate({ gross_weight: e.target.value })}
								placeholder="0.00"
								className={
									getFieldError("gross_weight") ? "border-red-500" : ""
								}
							/>
						...
						{/* Net Weight */}
						<div className="space-y-2">
							<Label htmlFor={`net_weight_${index}`}>Net Weight (g)</Label>
							<Input
								id={`net_weight_${index}`}
								type="number"
								step="0.01"
								value={bag.net_weight}
								onChange={(e) => onUpdate({ net_weight: e.target.value })}
								placeholder="0.00"
								className={getFieldError("net_weight") ? "border-red-500" : ""}
							/>
