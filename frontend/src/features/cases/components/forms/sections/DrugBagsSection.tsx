			...
			<CardContent className="space-y-6">
				...
				<div className="space-y-2 pb-4 border-b">
					...
					<Calendar22
						...
						onChange={(date) => formStore.updateField("assessment_date", date)}
						placeholder="Select assessment date"
					/>
					<p className="text-xs text-muted-foreground">
						Date of botanical assessment (defaults to received date or today)
					</p>
				</div>
				...
				<div className="space-y-2 pb-4">
					...
					/>
					<p className="text-xs text-muted-foreground">
						Auto-populated with subsample information. Edit as needed. This will
						appear in section (c) of the certificate.
					</p>
				</div>
					...
					<div className="flex items-center justify-between">
						<div>
							...
							<p className="text-sm text-muted-foreground">
								Add and manage drug bags for this case
							</p>
						</div>
						<Button
							type="button"
					...
					{formStore.formData.bags.length === 0 ? (
						<div className="p-8 border border-dashed rounded-lg text-center">
							...
							<p className="text-sm text-muted-foreground mb-4">
								No drug bags added yet. Click "Add Bag" to start adding bags.
							</p>
							<Button type="button" onClick={handleAddBag} variant="outline">
								<Plus className="h-4 w-4 mr-2" />
								Add First Bag
							</Button>
						</div>
						...
						<div className="space-y-4">
							{formStore.formData.bags.map((bag, index) => (
								<DrugBagFormItem
									...
									bag={bag}
									index={index}
									onUpdate={(data) => formStore.updateDrugBag(index, data)}
									onRemove={() => formStore.removeDrugBag(index)}
				...
				{formStore.formData.bags.length > 0 && (
					<div className="pt-4">
						<div className="flex items-center justify-between text-sm">
							...
							<span className="font-medium">{formStore.totalBags}</span>
						</div>
					</div>
				)}
