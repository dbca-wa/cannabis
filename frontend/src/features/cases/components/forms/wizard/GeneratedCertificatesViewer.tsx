			...
			>
				...
				<div className="flex items-center justify-end gap-1 border-b border-border bg-muted/40 px-3 py-1.5">
					...
					<Button
						...
						onClick={async () => {
							if (!activeCert || !caseId) return;
							try {
								const blob = await apiClient.getBlob(
									ENDPOINTS.CASES.DOCUMENTS.CERTIFICATE_PDF(
										caseId,
										activeCert.id
									)
								);
								...
								a.href = url;
								a.download = `unsigned_certificate_${activeCert.certificate_number || activeCert.id}.pdf`;
								document.body.appendChild(a);
								a.click();
								document.body.removeChild(a);
								URL.revokeObjectURL(url);
							} catch {
								// Fallback: if getBlob fails, try window.open
								window.open(
									ENDPOINTS.CASES.DOCUMENTS.CERTIFICATE_PDF(
										caseId,
										activeCert.id
