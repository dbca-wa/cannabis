...
import type { ViewMode } from "@/features/cases/stores/caseForm.store";

interface CaseFormLayoutProps {
...
export const CaseFormLayout: React.FC<CaseFormLayoutProps> = observer(
	({
		formContent,
		previewContent,
				...
				<div className="space-y-4 mb-6">
						...
						<div className="space-y-2">
							...
							{(formStore.isDirty || formStore.lastSaved) && (
								<div className="flex items-center gap-3 text-xs text-muted-foreground">
									{formStore.isDirty && <span>(Unsaved changes)</span>}
									{formStore.lastSaved && (
										<span>
											Last saved: {formStore.lastSaved.toLocaleTimeString()}
										</span>
									)}
								</div>
							)}
							...
							{!isMobile && !isTablet && (
								<div className="text-xs text-muted-foreground text-center">
									Keyboard shortcuts: Alt+1 (Form), Alt+2 (Both), Alt+3
									(Preview)
								</div>
							)}
						</div>
					...
					{(isMobile || isTablet) && showViewSwitcher && (
						<div className="text-xs text-muted-foreground p-2 bg-muted rounded">
							Dual view is disabled on mobile/tablet devices. Use Form or
							Preview modes, or switch to desktop for side-by-side view.
						</div>
					)}
				</div>
			);
...
CaseFormLayout.displayName = "CaseFormLayout";
