 ...
 *
 * Displays case summary with cost breakdown for finance review.
	...
	observer<FinanceApprovalPhaseContentProps>(
		({ caseObj, isCurrentPhase, canEdit, onUpdate }) => {
			...
			const [forensicHours, setForensicHours] = useState<string>(
				caseObj.forensic_hours || ""
			);
			const [fuelDistanceKm, setFuelDistanceKm] = useState<string>(
				caseObj.fuel_distance_km || ""
			);

			// Fetch system settings for pricing
			const { data: systemSettings } = useSystemSettings();

			// Calculate costs based on case data and system settings
			const calculateCosts = () => {
				if (!systemSettings) {
					// Return default values if settings not loaded
					return {
						bagProcessingFee: 0,
						bagCount: caseObj.bags.length,
						bagProcessingTotal: 0,
						certificateFee: 0,
						certificateCount: caseObj.certificates.length || 1,
						certificateTotal: 0,
						callOutFee: 0,
						callOutTotal: 0,
						...
						fuelDistanceValue: 0,
						fuelTotal: 0,
						additionalFees: caseObj.additional_fees,
				...
				const bagCount = caseObj.bags.length;
				const certificateCount = caseObj.certificates.length || 1;
				...
				// Additional fees from case (exclude fuel, call_out, forensic as they're handled separately)
				const filteredAdditionalFees = caseObj.additional_fees.filter(
					(fee) => !["fuel", "call_out", "forensic"].includes(fee.claim_kind)
				);
			...
			if (isCurrentPhase && canEdit) {
				...
				const formContent = (
					<div className="space-y-4">
							...
							<div className="flex items-center gap-2">
								...
								<div>
									...
									<p className="text-xs text-cyan-700 dark:text-cyan-300">
										Review the cost breakdown and invoice preview. Provide
										approval notes below.
									</p>
								</div>
							</div>
						</div>
							...
							<CardContent className="space-y-4">
								...
								<div className="space-y-3">
									...
									<div className="flex justify-between items-center pb-2 border-b border-gray-200 dark:border-gray-700">
										<div>
											...
											<p className="text-xs text-gray-500 dark:text-gray-400">
												{costs.certificateCount} certificate(s) × A$
									...
									<div className="flex justify-between items-center pb-2 border-b border-gray-200 dark:border-gray-700">
										...
										<p className="text-sm font-semibold text-gray-900 dark:text-gray-100">
											A$
											{costs.bagProcessingTotal.toFixed(2)}
										</p>
									</div>
										...
										<div className="flex-1">
											...
											<div className="flex items-center gap-2">
												<Input
													...
													min="0"
													value={forensicHours}
													onChange={(e) => setForensicHours(e.target.value)}
													placeholder="0.00"
													className="w-24"
												/>
												<span className="text-xs text-gray-500 dark:text-gray-400">
													hours × A$
													{costs.forensicHourlyRate.toFixed(2)}
													/hr
												</span>
											</div>
										</div>
										...
										<div className="flex-1">
											...
											<div className="flex items-center gap-2">
												<Input
													...
													onChange={(e) => setFuelDistanceKm(e.target.value)}
													placeholder="0.0"
													className="w-24"
												/>
												<span className="text-xs text-gray-500 dark:text-gray-400">
													km × A$
													{costs.fuelCostPerKm.toFixed(3)}
													/km
												</span>
											</div>
										</div>
											...
											{costs.additionalFees.map((fee) => (
												...
												>
													<div>
														...
														<p className="text-xs text-gray-500 dark:text-gray-400">
															{fee.description || `${fee.units} unit(s)`}
														</p>
													</div>
													<p className="text-sm font-semibold text-gray-900 dark:text-gray-100">
														A$
														{parseFloat(fee.calculated_cost).toFixed(2)}
													</p>
												</div>
											))}
										</>
						...
						<Card>
							<CardContent className="pt-6">
								<div className="flex items-center space-x-2">
									...
									>
										I approve the financial aspects of this case
									</Label>
								</div>
							</CardContent>
						</Card>
						...
						<div className="flex justify-end">
							<Button onClick={handleSave} disabled={isSaving || !isApproved}>
				...
				const previewContent = <InvoicePDFPreview caseObj={caseObj} />;
							...
							<div className="flex items-center gap-2">
								...
								<div>
									...
									<p className="text-xs text-gray-600">
										This is a read-only view of the finance approval phase. The
										case has moved to {caseObj.phase_display}.
									</p>
								</div>
							</div>
						</div>
								...
								<div className="flex items-start gap-3">
									...
									<div className="flex-1">
										...
										<p className="text-xs text-green-700 dark:text-green-300 mt-1">
											Approved by{" "}
											{caseObj.finance_officer_details?.full_name || "Unknown"}{" "}
											on{" "}
											{new Date(caseObj.finance_approved_at).toLocaleString()}
										</p>
									</div>
								</div>
							</CardContent>
						...
						<CardContent className="space-y-4">
							<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
								...
								<div>
									...
									<p className="text-base text-gray-900 dark:text-gray-100">
										{caseObj.requesting_officer_details?.full_name ||
											"Not assigned"}
									</p>
								</div>
								<div>
									...
									<p className="text-base text-gray-900 dark:text-gray-100">
										{caseObj.station_details?.name || "Not assigned"}
									</p>
								</div>
							</div>
						</CardContent>
					...
					<Card>
						<CardHeader>
							<CardTitle className="flex items-center gap-2">
								...
								Drug Bags ({caseObj.bags.length})
							</CardTitle>
						</CardHeader>
						<CardContent>
							{caseObj.bags.length > 0 ? (
								<div className="space-y-3">
									{caseObj.bags.map((bag, index) => (
										<div
											key={bag.id}
					...
					<Card>
						<CardHeader>
							<CardTitle className="flex items-center gap-2">
								...
								Defendants ({caseObj.defendants_details.length})
							</CardTitle>
						</CardHeader>
						<CardContent>
							{caseObj.defendants_details.length > 0 ? (
								<div className="space-y-2">
									{caseObj.defendants_details.map((defendant) => (
										<div
											key={defendant.id}
