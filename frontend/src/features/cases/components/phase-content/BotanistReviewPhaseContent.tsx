	...
	observer<BotanistReviewPhaseContentProps>(
		({ caseObj, isCurrentPhase, canEdit }) => {
			...
			const [assessmentDate, setAssessmentDate] = useState<string>(
				caseObj.bags[0]?.assessment?.assessment_date?.split("T")[0] ||
					new Date().toISOString().split("T")[0]
			);

			// Check if all bags have been assessed (determination is not "pending")
			const allBagsAssessed = caseObj.bags.every(
				(bag) =>
							...
							<div className="flex items-center gap-2">
								...
								<div>
									...
									<p className="text-xs text-green-700 dark:text-green-300">
										Review and assess each drug bag below. All bags must be
										assessed before advancing to the next phase.
									</p>
								</div>
							</div>
						</div>
								...
								<div className="flex items-center gap-2">
									...
									<div>
										...
										<p className="text-xs text-amber-700 dark:text-amber-300">
											All drug bags must have a determination selected before
											you can advance to the Documents phase.
										</p>
									</div>
								</div>
							</div>
						...
						<Card>
							<CardHeader>
								<CardTitle className="flex items-center gap-2">
									...
									Drug Bags Assessment ({caseObj.bags.length})
								</CardTitle>
							</CardHeader>
							<CardContent>
											...
											return (
												...
												>
													...
													<div className="space-y-2">
														...
														>
															<SelectTrigger id={`determination-${bag.id}`}>
																<SelectValue placeholder="Select determination" />
															</SelectTrigger>
															<SelectContent>
																{DETERMINATION_CHOICES.map((choice) => (
																	<SelectItem
																		key={choice.value}
																		value={choice.value}
																	>
													...
													{/* Botanist Notes */}
													<div className="space-y-2">
														<Label htmlFor={`notes-${bag.id}`}>
															Botanist Notes (Optional)
														</Label>
														<Textarea
															id={`notes-${bag.id}`}
															value={botanistNotes}
							...
							<div className="flex items-center gap-2">
								...
								<div>
									...
									<p className="text-xs text-gray-600">
										This is a read-only view of the botanist review phase. The
								...
								<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
									<div>
										...
										<p className="text-base text-gray-900 dark:text-gray-100">
											{caseObj.approved_botanist_details?.full_name ||
												"Unknown"}
										</p>
									</div>
									<div>
										...
										<p className="text-base text-gray-900 dark:text-gray-100">
											{new Date(caseObj.botanist_approved_at).toLocaleString()}
										</p>
									</div>
								</div>
							</CardContent>
									...
									{caseObj.bags.map((bag, index) => (
										...
										>
											...
											<div className="flex items-start justify-between">
												...
												<Badge variant="outline">{bag.seal_tag_numbers}</Badge>
											</div>
