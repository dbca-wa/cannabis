...
import { useCertificatePreviewStore } from "@/features/cases/hooks/useCertificatePreviewStore";
import { FileText } from "lucide-react";
import { cn } from "@/shared/utils/style.utils";
import { formatDate } from "@/shared/utils/date.utils";
import type { DefendantTiny } from "@/shared/types/backend-api.types";
import type { DrugBagFormData } from "@/features/cases/stores/caseForm.store";
			...
			return (
				...
				>
					...
					<p className="text-sm text-muted-foreground max-w-md">
						Complete the required fields (Case Number, Received Date, and
						Envelope Number) to see the certificate preview.
					</p>
				</div>
			);
		}
		...
		return (
			...
			>
				...
				<div className="space-y-3">
					...
					<div className="grid grid-cols-2 gap-4 text-sm">
						...
						<div>
							...
							<p className="text-muted-foreground">
								{data.received_date ? formatDate(data.received_date) : "N/A"}
							</p>
						</div>
					</div>
				</div>
				...
				{(data.requesting_officer ||
					...
					<div className="space-y-3">
						...
						<div className="grid grid-cols-1 gap-3 text-sm">
							{data.requesting_officer && (
								<div>
									<span className="font-medium">Requesting Officer:</span>
									<p className="text-muted-foreground">
							...
							{data.submitting_officer && (
								<div>
									<span className="font-medium">Submitting Officer:</span>
									<p className="text-muted-foreground">
							...
							{data.station && (
								<div>
									...
									<p className="text-muted-foreground">{data.station.name}</p>
								</div>
							)}
						</div>
					</div>
				...
				{data.defendants.length > 0 && (
					<div className="space-y-3">
						<h2 className="text-lg font-semibold border-b pb-2">Defendants</h2>
				...
				<div className="space-y-3">
					...
					<div className="grid grid-cols-2 gap-4 text-sm">
						<div>
							...
							<p className="text-muted-foreground">{data.total_bags}</p>
						</div>
						<div>
							<span className="font-medium">Cannabis Present:</span>
							<p
								className={cn(
									"font-medium",
				...
				{data.bags.length > 0 && (
					<div className="space-y-3">
						...
						<div className="space-y-4">
							{data.bags.map((bag: DrugBagFormData, index: number) => (
								<div
									key={index}
									className="border rounded-lg p-4 space-y-2 text-sm"
								>
									<div className="flex items-center justify-between">
										<span className="font-medium">Bag {index + 1}</span>
										<span className="text-xs text-muted-foreground">
											{bag.content_type.replace("_", " ").toUpperCase()}
										</span>
									</div>
									{bag.seal_tag_numbers && (
										<div>
											<span className="font-medium">Seal Tags:</span>
											<p className="text-muted-foreground">
												{bag.seal_tag_numbers}
											</p>
										</div>
									...
									{bag.determination && bag.determination !== "pending" && (
										<div>
											...
											<p
												className={cn(
													"font-medium",
													bag.determination.includes("cannabis")
														? "text-green-600 dark:text-green-400"
														: "text-gray-600 dark:text-gray-400"
												)}
											>
				...
				{data.approved_botanist && (
					<div className="space-y-3">
						<h2 className="text-lg font-semibold border-b pb-2">Approved By</h2>
						<div className="text-sm">
							<p className="font-medium">{data.approved_botanist.full_name}</p>
							<p className="text-muted-foreground">
				...
				<div className="border-t pt-4 text-xs text-center text-muted-foreground">
					<p>This certificate is generated by the Cannabis Management System</p>
