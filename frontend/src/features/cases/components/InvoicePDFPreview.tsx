...
import {
	generateInvoiceHTML,
	generateInvoiceData,
} from "@/features/cases/utils/invoiceTemplate";
...
export const InvoicePDFPreview: React.FC<InvoicePDFPreviewProps> = ({
	...
	// Generate HTML preview when case or settings change
	const generatePreview = useCallback(async () => {
		...
		try {
			const invoiceData = generateInvoiceData(caseObj, systemSettings);
		...
		<div className={cn("w-full h-full", className)}>
			...
			<div className="w-full h-full overflow-auto bg-gray-100 dark:bg-gray-800 p-6">
				{error ? (
					...
					<div className="flex flex-col items-center justify-center h-full">
						<Loader className="h-12 w-12 text-muted-foreground mb-4 animate-spin" />
						<h3 className="text-lg font-medium mb-2">Generating Preview</h3>
