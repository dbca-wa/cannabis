...
import { type CasePhase, type Case } from "@/shared/types/backend-api.types";
...
export const PhaseContent = observer<PhaseContentProps>(
	({ caseObj, phase, isCurrentPhase, canEdit, onUpdate, className = "" }) => {
		// Determine if we're viewing historical data
		const isHistoricalView = !isCurrentPhase;

		// Get phase display name
		const phaseDisplayName = getPhaseDisplay(phase);
				...
				default:
					return (
						<div className="p-8 text-center">
							...
							<p className="text-gray-600">Unknown phase: {phase}</p>
						</div>
					);
			}
		};
						...
						<div className="flex items-center gap-2">
							...
							<div>
								...
								<p className="text-xs text-blue-700 dark:text-blue-300 mt-1">
									You are viewing the {phaseDisplayName} phase as it was
									completed. This data is read-only.
								</p>
							</div>
						</div>
					</div>
						...
						<div className="flex items-center gap-2">
							...
							<div>
								...
								<p className="text-xs text-amber-700 dark:text-amber-300 mt-1">
									You do not have permission to edit this phase.
								</p>
							</div>
						</div>
					</div>
