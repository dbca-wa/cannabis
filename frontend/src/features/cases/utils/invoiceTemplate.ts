import type { Case, SystemSettings } from "@/shared/types/backend-api.types";
...
export const generateInvoiceData = (
	...
	const forensicHourlyRate = parseFloat(systemSettings.cost_per_forensic_hour);
	...
	services.push({
		...
		description: `Botanical identification certificate for case ${caseObj.case_number}`,
	...
	const forensicHours = parseFloat(caseObj.forensic_hours || "0");
	if (forensicHours > 0) {
		services.push({
			name: "Forensic Work",
	...
	const fuelDistanceKm = parseFloat(caseObj.fuel_distance_km || "0");
	if (fuelDistanceKm > 0) {
		services.push({
			name: "Fuel Cost",
	...
	// Get officer info
	const officer =
		caseObj.requesting_officer_details || caseObj.submitting_officer_details;
	const officerName = officer?.full_name || "Unknown Officer";
	const officerId = officer?.badge_number || "N/A";

	// Get station info
	const station = caseObj.station_details;

	return {
		invoice_id: `INV${new Date().getFullYear()}-${String(caseObj.id).padStart(
			3,
			"0"
		)}`,
		...
		approved_botanist:
			caseObj.approved_botanist_details?.full_name || "Not assigned",
		finance_officer:
				...
				<table class="invoice-table">
					...
					<tbody>
						...
						<tr class="service-row ${
							index === data.services.length - 1 ? "last-service" : ""
						}">
							<td class="item-cell">
