...
interface ResponsiveModalOverlayProps extends React.ComponentPropsWithoutRef<
...
const ResponsiveModalVariants = cva(
	cn(
		...
		"lg:left-[50%] lg:top-[50%] lg:w-full lg:max-w-lg lg:translate-x-[-50%] lg:translate-y-[-50%] lg:border lg:duration-200 lg:data-[state=open]:animate-in lg:data-[state=closed]:animate-out lg:data-[state=closed]:fade-out-0 lg:data-[state=open]:fade-in-0 lg:data-[state=closed]:zoom-out-95 lg:data-[state=open]:zoom-in-95 lg:rounded-xl"
	),
	{
		variants: {
			side: {
...
>(
	(
		...
		<ResponsiveModalPortal>
			<ResponsiveModalOverlay hideOverlay={hideOverlay} />
			...
			>
				...
				{children}
				<ResponsiveModalClose className="absolute z-[99999] right-4 top-4 rounded-sm opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none data-[state=open]:bg-secondary">
					<X className="h-4 w-4" />
