...
class LoggerService {
	...
	private formatTimestamp(): string {
		try {
			...
			try {
				...
				console.warn('Logger: Perth timezone formatting failed, using local timezone');
				return `${hour}:${minute}:${second} LOCAL`;
			} catch (fallbackError) {
				// Final fallback to ISO string if all formatting fails
				console.warn('Logger: All timezone formatting failed, using ISO string');
				return new Date().toISOString();
			}
		}
	}

	private formatContext(context?: LogContext): { hasContext: boolean; contextObject?: LogContext } {
	...
	private formatMessage(
		...
		switch (level) {
			case 'error':
				headerStyle = 'background: #dc2626; color: white; padding: 2px 6px; border-radius: 3px; font-weight: bold;'; // Red
				break;
			case 'warn':
				headerStyle = 'background: #d97706; color: white; padding: 2px 6px; border-radius: 3px; font-weight: bold;'; // Yellow/Orange
				break;
			case 'info':
			case 'debug':
			default:
				headerStyle = 'background: #2563eb; color: white; padding: 2px 6px; border-radius: 3px; font-weight: bold;'; // Blue
				break;
		}
		...
		return {
			...
			contextObject: contextInfo.hasContext ? contextInfo.contextObject : undefined
		};
	}

	debug(message: string, context?: LogContext): void {
		if (this.isDev) {
			const { formattedArgs, contextObject } = this.formatMessage("debug", message, context);
			if (contextObject) {
	...
	info(message: string, context?: LogContext): void {
		const { formattedArgs, contextObject } = this.formatMessage("info", message, context);
		if (contextObject) {
	...
	warn(message: string, context?: LogContext): void {
		const { formattedArgs, contextObject } = this.formatMessage("warn", message, context);
		if (contextObject) {
	...
	error(message: string, context?: LogContext): void {
		const { formattedArgs, contextObject } = this.formatMessage("error", message, context);
		if (contextObject) {
