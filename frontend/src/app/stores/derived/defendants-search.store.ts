...
export type DefendantsSearchFilters = Record<string, never>;

interface DefendantsSearchStoreState extends BaseStoreState {
	searchTerm: string;
	filters: DefendantsSearchFilters;
	currentPage: number;
	totalPages: number;
	totalResults: number;
	saveSearch: boolean;
...
const STORAGE_KEY = "defendantsSearchState";

const DEFENDANTS_DEFAULT_FILTERS: DefendantsSearchFilters = {};

const INITIAL_STATE: DefendantsSearchStoreState = {
	loading: false,
	error: null,
	initialised: false,
	searchTerm: "",
	filters: { ...DEFENDANTS_DEFAULT_FILTERS },
	currentPage: 1,
	totalPages: 1,
	totalResults: 0,
	saveSearch: true,
...
export class DefendantsSearchStore extends BaseStore<DefendantsSearchStoreState> {
	constructor() {
		super({ ...INITIAL_STATE });
	 ...
	 */
	setFilters(filters: Partial<DefendantsSearchFilters>) {
		this.state.filters = {
			...this.state.filters,
			...filters,
		} as DefendantsSearchFilters;
		this.setCurrentPage(1);
	}
	 ...
	 */
	resetFilters() {
		this.state.filters = { ...DEFENDANTS_DEFAULT_FILTERS };
	...
	private loadFromStorage() {
		try {
			...
			logger.warn("Failed to load defendants search state from localStorage", {
	...
	private saveToStorage() {
		if (this.state.saveSearch) {
			try {
				...
				logger.warn("Failed to save defendants search state to localStorage", {
					error: error instanceof Error ? error.message : String(error),
				});
			}
		} else {
...
export const defendantsSearchStore = new DefendantsSearchStore();
