...
export interface OfficersSearchFilters {
	station: number | null;
	rank: string;
}

interface OfficersSearchStoreState extends BaseStoreState {
	searchTerm: string;
	filters: OfficersSearchFilters;
	currentPage: number;
	totalPages: number;
	totalResults: number;
	saveSearch: boolean;
...
const STORAGE_KEY = "officersSearchState";

const OFFICERS_DEFAULT_FILTERS: OfficersSearchFilters = {
	station: null,
	rank: "all",
};

const INITIAL_STATE: OfficersSearchStoreState = {
	loading: false,
	error: null,
	initialised: false,
	searchTerm: "",
	filters: { ...OFFICERS_DEFAULT_FILTERS },
	currentPage: 1,
	totalPages: 1,
	totalResults: 0,
	saveSearch: true,
...
export class OfficersSearchStore extends BaseStore<OfficersSearchStoreState> {
	constructor() {
		super({ ...INITIAL_STATE });
	 ...
	 */
	setFilters(filters: Partial<OfficersSearchFilters>) {
	 ...
	 */
	resetFilters() {
		this.state.filters = { ...OFFICERS_DEFAULT_FILTERS };
	 ...
	 */
	reset() {
		this.state = { ...INITIAL_STATE, filters: { ...OFFICERS_DEFAULT_FILTERS } };
	}
	...
	get filterCount(): number {
		...
		if (this.state.filters.station !== null) count++;
		if (this.state.filters.rank !== "all") count++;
	...
	get searchParams(): URLSearchParams {
		...
		if (this.state.filters.rank !== "all") {
			params.set("rank", this.state.filters.rank);
		}
	...
	private loadFromStorage() {
		try {
			...
			logger.warn("Failed to load officers search state from localStorage", {
			...
			this.state.filters = {
				station:
					typeof filters.station === "number"
						? filters.station
						: OFFICERS_DEFAULT_FILTERS.station,
				rank:
					typeof filters.rank === "string"
						? filters.rank
						: OFFICERS_DEFAULT_FILTERS.rank,
			};
		}
	...
	private saveToStorage() {
		if (this.state.saveSearch) {
			try {
				...
				logger.warn("Failed to save officers search state to localStorage", {
					error: error instanceof Error ? error.message : String(error),
				});
			}
		} else {
...
export const officersSearchStore = new OfficersSearchStore();
