...
export type StationsSearchFilters = Record<string, never>;

interface StationsSearchStoreState extends BaseStoreState {
	searchTerm: string;
...
const STORAGE_KEY = "stationsSearchState";

const STATIONS_DEFAULT_FILTERS: StationsSearchFilters = {};

const INITIAL_STATE: StationsSearchStoreState = {
	loading: false,
	error: null,
	initialised: false,
	searchTerm: "",
	filters: { ...STATIONS_DEFAULT_FILTERS },
	currentPage: 1,
	totalPages: 1,
	totalResults: 0,
	saveSearch: true,
...
export class StationsSearchStore extends BaseStore<StationsSearchStoreState> {
	constructor() {
		super({ ...INITIAL_STATE });
	 ...
	 */
	setFilters(filters: Partial<StationsSearchFilters>) {
		this.state.filters = {
			...this.state.filters,
			...filters,
		} as StationsSearchFilters;
		this.setCurrentPage(1);
	}
	 ...
	 */
	resetFilters() {
		this.state.filters = { ...STATIONS_DEFAULT_FILTERS };
	 ...
	 */
	reset() {
		this.state = { ...INITIAL_STATE, filters: { ...STATIONS_DEFAULT_FILTERS } };
	}
	...
	private loadFromStorage() {
		try {
			...
			logger.warn("Failed to load stations search state from localStorage", {
	...
	private saveToStorage() {
		if (this.state.saveSearch) {
			try {
				...
				logger.warn("Failed to save stations search state to localStorage", {
					error: error instanceof Error ? error.message : String(error),
				});
			}
		} else {
...
export const stationsSearchStore = new StationsSearchStore();
