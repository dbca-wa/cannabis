...
export default function InviteActivation() {
    ...
    const [activationState, setActivationState] = useState<ActivationState>({ status: 'loading' });
    ...
    const activateInvitation = async (inviteToken: string) => {
        try {
            logger.info("Activating invitation", { token: inviteToken.substring(0, 8) + "..." });
                ...
                return (
                    <Card className="w-full max-w-md mx-auto">
                        <CardHeader className="text-center">
                            ...
                            <CardTitle className="text-green-700">Welcome to Cannabis Management System!</CardTitle>
                            <CardDescription>
                        ...
                        <CardContent className="space-y-4">
                            ...
                            <div className="text-center">
                                <p className="text-sm text-muted-foreground mb-4">
                                    Welcome, <strong>{activationState.data.user.full_name}</strong>!<br />
                                    Role: <strong>{activationState.data.user.role}</strong>
                                </p>

                                <Button
                                    onClick={() => navigate('/auth/password-update', {
                                        replace: true,
                                        state: {
                                            isFirstTime: true,
                                            temporaryPassword: activationState.data.temporary_password,
                                            fromInvitation: true
                                        }
                                    })}
                                    className="w-full"
                ...
                return (
                    <Card className="w-full max-w-md mx-auto">
                        <CardHeader className="text-center">
                            ...
                            <CardTitle className="text-red-700">Invitation Activation Failed</CardTitle>
                            <CardDescription>
                        ...
                        <CardContent className="space-y-4">
                            ...
                            <div className="text-sm text-muted-foreground text-center">
                                ...
                                {activationState.error.toLowerCase().includes('used') && (
                                    <p>This invitation has already been used. If you have an account, try logging in.</p>
                                )}
                                {activationState.error.toLowerCase().includes('invalid') && (
                                    <p>This invitation link appears to be invalid. Please check the link or contact your administrator.</p>
                                )}
                            </div>
                        </CardContent>
                    </Card>
