...
if command -v poetry &> /dev/null; then
    run_check "Flake8 (linting)" "poetry run flake8 --max-line-length=88 --extend-ignore=E203,W503,E501 --exclude=migrations,__pycache__,.venv,venv,build,dist,.pytest_cache,htmlcov,staticfiles,etl,scripts,_run_etl.py,_etl_ ."
else
...
# Run bandit (security)
if command -v poetry &> /dev/null; then
    run_check "Bandit (security)" "poetry run bandit -c pyproject.toml -r . --exclude tests,migrations,venv,scripts,_run_etl.py -q"
else
