...
class PoliceOfficerSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        read_only_fields = ['id', 'created_at', 'updated_at', 'full_name', 'is_sworn', 'rank_display']
    
    def validate_badge_number(self, value):
        """Ensure badge numbers are unique when provided"""
        if value and PoliceOfficer.objects.filter(badge_number=value).exclude(pk=self.instance.pk if self.instance else None).exists():
...
class PoliceOfficerCreateSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        if not attrs.get('first_name') and not attrs.get('last_name'):
            raise serializers.ValidationError("At least first name or last name must be provided.")
        return attrs