...
class BruteForceProtectionThrottle:
    ...
    def check_brute_force_protection(
        ...
        if email_lockout:
            remaining_time = (
                email_lockout.get("expires_at") - timezone.now().timestamp()
            )
            if remaining_time > 0:
                return (
                    False,
                    "Email temporarily locked due to too many failed attempts",
                    int(remaining_time),
                )
        ...
        if ip_lockout:
            ...
            if remaining_time > 0:
                return (
                    False,
                    "IP address temporarily locked due to too many failed attempts",
                    int(remaining_time),
                )
