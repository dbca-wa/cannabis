...
class UserFriendlyMessages:
    ...
    INVALID_TOKEN = (
        "This link is invalid or has been tampered with. Please request a new one."
    )
    ...
    PASSWORD_VALIDATION_FAILED = (
        "Password validation failed. Please check the requirements and try again."
    )
...
class SecurityEventLogger:
    ...
    def log_invitation_sent(
        inviter_email: str, invited_email: str, role: str, token_prefix: str
    ):
    ...
    def log_reset_code_brute_force_attempt(
        email: str, attempt_count: int, ip_address: str = None
    ):
...
class ErrorResponseBuilder:
    ...
    def build_error_response(
        ...
        """
        Build a standardised error response using DRF conventions

        Args:
            error_code: Standardised error code (included as extra data)
...
class InvitationErrorHandler:
    ...
    def handle_user_already_exists(email: str) -> Response:
        ...
        SecurityEventLogger.log_invitation_activation_failed(
            email=email, reason="user_already_exists", token_prefix="N/A"  # nosec B106
        )
    ...
    def handle_invalid_token(token_prefix: str, reason: str) -> Response:
        ...
        SecurityEventLogger.log_invitation_activation_failed(
            email="unknown", reason=f"invalid_token_{reason}", token_prefix=token_prefix
        )
...
class PasswordErrorHandler:
    ...
    def handle_incorrect_current_password(email: str) -> Response:
        ...
        SecurityEventLogger.log_password_update_failed(
            email=email, reason="incorrect_current_password"
        )
        ...
        return ErrorResponseBuilder.build_error_response(
            ...
            field_errors={
                "current_password": [UserFriendlyMessages.CURRENT_PASSWORD_INCORRECT]
            },
            status_code=status.HTTP_400_BAD_REQUEST,
        )
    ...
    def handle_passwords_dont_match() -> Response:
        ...
        return ErrorResponseBuilder.build_error_response(
            ...
            field_errors={
                "confirm_password": [UserFriendlyMessages.PASSWORDS_DONT_MATCH]
            },
            status_code=status.HTTP_400_BAD_REQUEST,
        )
