...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--ip", type=str, help="IP address to unlock (use with --action unlock)"
        )
        parser.add_argument(
            "--format",
            type=str,
    ...
    def get_lockout_info(self):
        ...
        lockouts = {"email_lockouts": [], "ip_lockouts": []}
    ...
    def display_table_format(self, lockouts, rate_limits):
        ...
        self.stdout.write("-" * 30)
        self.stdout.write(
            "Use Django admin or cache inspection tools for detailed rate limit counters."
        )

    def clear_all_limits(self):
        """Clear all rate limits and lockouts"""
        if not self.confirm_action(
            "This will clear ALL rate limits and lockouts. Continue?"
        ):
            return
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    "All cache cleared - rate limits and lockouts reset!"
                )
            )
        except Exception as e:
    ...
    def unlock_specific(self, email, ip_address):
        ...
        if unlocked:
            ...
            self.stdout.write(self.style.WARNING("No lockouts found to clear"))
