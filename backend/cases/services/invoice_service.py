...
class InvoiceService:
    ...
    @staticmethod
    def build_invoice_context(submission, invoice, line_items=None, tax_enabled=True):
        ...
        When line_items is provided, renders the services list from the override
        data instead of auto-detecting from submission bags/certs/fees.
        ...
        if line_items is not None:
            # Render services from the provided line items
            fee_type_labels = {
                "certificate": "Certificate of Approved Botanist",
                "bag": "Botanical Identification",
                "fuel": "Fuel/Travel",
                "call_out": "Call Out",
            ...
            for item in line_items:
                ...
                line_total = (quantity * rate).quantize(
                    Decimal("0.01"), rounding=ROUND_HALF_UP
                )
                services.append(
                    {
                        "name": fee_type_labels.get(fee_type, fee_type),
                        "description": "",
                ...
                services.append(
                    {
                        ...
                        "line_total": float(
                            cert_count * settings_obj.cost_per_certificate
                        ),
                    }
                )
    ...
    def generate_invoice(
        submission, customer_number, user, line_items=None, tax_enabled=True
    ):
        ...
        When line_items is provided (list of dicts with fee_type, quantity, rate),
        uses those items instead of auto-detecting from case data. For fuel,
        call_out, and forensic items, creates AdditionalInvoiceFee records.

        Args:
            submission: The Case instance.
            customer_number: Customer reference number.
            user: The user performing the action.
            line_items: Optional list of dicts with keys: fee_type, quantity, rate.
        ...
        if line_items is not None:
            # Use provided line items for calculations
            ...
            # Create AdditionalInvoiceFee records for fuel, call_out, forensic
            additional_fee_types = {"fuel", "call_out", "forensic"}
            for item in line_items:
                fee_type = item["fee_type"]
                if fee_type in additional_fee_types:
                    AdditionalInvoiceFee.objects.create(
                        submission=submission,
                        claim_kind=fee_type,
                        units=Decimal(str(item["quantity"])),
                    )

            # Build context using the provided line items
            context = InvoiceService.build_invoice_context(
                submission, invoice, line_items=line_items, tax_enabled=tax_enabled
            )
        else:
            # Existing auto-detection behaviour (backward compatible)
            invoice = Invoice.objects.create(
                submission=submission,
                customer_number=customer_number,
            )
...
def build_invoice_context(submission, invoice, line_items=None, tax_enabled=True):
    ...
    return InvoiceService.build_invoice_context(
        submission, invoice, line_items=line_items, tax_enabled=tax_enabled
    )
